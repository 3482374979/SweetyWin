...
public sealed class SelectionService
{
    ...
    private static readonly TimeSpan ClipboardTimeout = TimeSpan.FromMilliseconds(600);
            ...
            var work = Task.Run(() =>
            {
                try
                {
                        ...
                        && patternObj is TextPattern textPattern)
                    {
                        ...
                        LogService.Log("UIA: TextPattern empty selection");
                        return string.Empty;
                    }
                    LogService.Log($"UIA: TextPattern not supported by {focused.Current.ControlType?.ProgrammaticName}");
                    return string.Empty;
                }
                catch (Exception ex)
                {
